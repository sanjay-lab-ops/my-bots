...
def _handle_command(text: str):
    ...
    if cmd == "/close":
        ...
        try:
            ...
            if not pos:
                _send(f"Ticket {ticket} not found in open positions.")
                return
            ...
            if result and result.retcode == mt5.TRADE_RETCODE_DONE:
                ...
                _send(f"❌ Close failed: {result.comment if result else 'unknown error'}")
        except Exception as e:
            _send(f"Error: {e}")
        return
